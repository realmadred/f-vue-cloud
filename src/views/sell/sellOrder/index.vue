	...
	<div class="SellOrder-container">
		<el-card shadow="hover">
			...
			<el-table :data="tableData.data" style="width: 100%">
				...
				<el-table-column prop="payAmount" label="首付金额" />
				<el-table-column prop="payMethod" label="支付方式" />
				<el-table-column prop="discountAmount" label="优惠金额" />
				<el-table-column prop="lastPayAmount" label="尾款金额" />
				<el-table-column prop="payTime" label="支付时间" />
				<el-table-column prop="status" label="状态" />
				<el-table-column prop="communityName" label="小区名称" />
				<el-table-column prop="memberName" label="业主姓名" />
				<el-table-column prop="memberPhone" label="业主电话" />
		...
		<AddSellOrder ref="addRef" />
		<EditSellOrder ref="editRef" />
	</div>
</template>
...
import AddSellOrder from '/@/views/sell/sellOrder/component/add.vue';
import EditSellOrder from '/@/views/sell/sellOrder/component/edit.vue';
import { selectPage, deleteById } from '/@/api/sell/sellOrder';
export default {
	name: 'SellOrder',
	components: { AddSellOrder, EditSellOrder },
	setup() {
		const addRef = ref();
		const editRef = ref();
